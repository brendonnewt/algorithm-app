...
use implementations::{bubble_sort, insertion_sort, quicksort, selection_sort};
...
pub fn get_output(input: SortingInput) -> Result<SortingOutput, HttpResponse> {
    ...
    match sort {
        ...
        Some(SortType::SelectionSort) => Ok(selection_sort(input)),
        Some(SortType::QuickSort) => Ok(quicksort(input)),
